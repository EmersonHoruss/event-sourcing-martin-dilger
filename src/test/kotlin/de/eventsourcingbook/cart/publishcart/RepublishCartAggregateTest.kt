...
class RepublishCartAggregateTest {
  ...
  fun `RepublishCartAggregateTest`() {
    ...
    events.add(RandomData.newInstance<CartPublishedEvent> { this.aggregateId = aggregateId })
    ...
    expectedEvents.add(
            RandomData.newInstance<CartPublicationFailedEvent> { this.aggregateId = aggregateId }
    )
