...
class ItemAddedEventUpcasterV1V2 : SingleEventUpcaster() {
        ...
        ): Boolean = intermediateRepresentation?.type?.equals(SOURCE_TYPE) ?: false
                ...
                intermediateRepresentation.upcastPayload(
                        ...
                        { item ->
                                (item as ObjectNode).put(
                                        de.eventsourcingbook.cart.events
                                                        .ItemAddedEvent::deviceFingerPrint
                                                .name,
        ...
        companion object {
                val SOURCE_TYPE: SimpleSerializedType =
                        SimpleSerializedType(
                                de.eventsourcingbook.cart.events.versioned.ItemAddedEvent::class
                                        .java.getTypeName(),
                                null
                        )
                val TARGET_TYPE: SimpleSerializedType =
                        SimpleSerializedType(
                                de.eventsourcingbook.cart.events.ItemAddedEvent::class.java
                                        .getTypeName(),
                                "2.0"
                        )
        }
